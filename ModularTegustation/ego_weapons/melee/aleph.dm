...
/obj/item/ego_weapon/shield/censored
	name = "数据删除"
	desc = "(数据删除) 能够 (数据删除), 但是在场的人将会目睹到令人作呕的景象. \
			盯着这把武器超过三秒就会使你犯恶心."
	special = "这把武器在成功格挡后会发动反击."
	icon_state = "censored"
	...
	damtype = BLACK_DAMAGE
	swingstyle = WEAPONSWING_THRUST
	attack_verb_continuous = list("(数据删除)")
	attack_verb_simple = list("(数据删除)")
	...
	block_message = "你的武器将尝试 (数据删除) 这次攻击!"
	projectile_block_message = "你 (数据删除) 了投射物!"
...
/obj/item/ego_weapon/shield/censored/attack_self(mob/user)
	...
	if (block == 0)
		...
		if(shield_user.physiology.armor.bomb) //"We have NOTHING that should be modifying this, so I'm using it as an existant parry checker." - Ancientcoders
			to_chat(shield_user,span_warning("你尚未恢复姿态平衡!"))
			return FALSE
		for(var/obj/machinery/computer/abnormality/AC in range(1, shield_user))
			if(AC.datum_reference.working) // No blocking during work.
				to_chat(shield_user,span_notice("你无法在工作时进行防御!"))
				return FALSE
...
/obj/item/ego_weapon/shield/censored/proc/Reflect(mob/living/carbon/human/user, mob/living/attacker)
	...
	attacker.visible_message(span_danger("[user]的 [src] (数据删除) 了 [attacker]!"), \
					span_userdanger("[user]的 [src] (数据删除) 了你!"), vision_distance = COMBAT_MESSAGE_RANGE, ignored_mobs = user)
	to_chat(user, span_danger("你的 [src] (数据删除) 了 [attacker]!"))
