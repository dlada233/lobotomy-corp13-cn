...
/obj/item/portacopier/attackby(obj/item/W, mob/user)
	if(istype(W, /obj/item/paper))
		if(paperstock <= 6)
			to_chat(user, span_notice("[src]嗡嗡作响."))
	...
	if(proximity_flag == 1)
		if(istype(target, /obj/item/paper))
			if(paperstock > 0)
				...
				paperstock--
				to_chat(user, span_notice("[src]嗡嗡作响，发出打字机般的声音."))
				playsound(get_turf(src), 'sound/effects/servostep.ogg', 50, TRUE)
				return
			else
				to_chat(user, span_notice("[src]发出机械卡壳声，像是某种东西用完了."))
				return
...
/obj/item/safety_kit/proc/ChangeMode(mob/user)
	...
	switch(mode)
		...
		if(RAK_RAPID_MODE)
			to_chat(user, span_notice("You will now cause regenerators to pulse quickly at low output."))
...
/obj/item/info_printer/examine(mob/user)
	...
	. = ..()
	. += "对异想体使用将在 [total_use_time/10] 秒后在屏幕上显示管理信息."
	if(use_time >= 10)
		. += "高谨慎将使该设备运行更快."
