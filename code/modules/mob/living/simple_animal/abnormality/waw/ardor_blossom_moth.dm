/mob/living/simple_animal/hostile/abnormality/ardor_moth
	...
	observation_prompt = "橙色光圈在你眼前悬浮飘动。<br>\
		光芒在空中闪烁舞动，形成朦胧雾霭。<br>\
		其中有某种存在正被灼烧致死。<br>\
		若被火焰触及，你是否也会化为焦炭？"
	observation_choices = list(
		"伸手触碰" = list(TRUE, "在光晕诱惑下，你伸出一根手指，<br>\
			等待着某个光点降落。<br>\
			闪耀的光球轻轻停驻指尖。<br>\
			随后，火焰吞噬了它。<br>\
			又一个光斑附着于身，接着四个，八个。<br>\
			它们不断增殖，直至你完全被光芒笼罩。"),
		"转身离开" = list(FALSE, "强忍触碰的冲动，<br>\
			你决定远离这可疑的温暖。<br>\
...
/mob/living/simple_animal/hostile/abnormality/ardor_moth/proc/FireSlash(target)
	...
	for(var/turf/T in area_of_effect)
		...
		for(var/mob/living/L in T)
			...
			L.visible_message(span_danger("[src] [attack_verb_continuous] [L]!"), \
					span_userdanger("[src] [attack_verb_continuous] 你!"), null, COMBAT_MESSAGE_RANGE, src)
			to_chat(src, span_danger("你 [attack_verb_simple] [L]!"))
...
/mob/living/simple_animal/hostile/abnormality/ardor_moth/proc/DeathExplosion()
	...
	visible_message(span_userdanger("[src]的火焰瞬间爆发!"), \
						span_userdanger("你已准备好精彩亮相!"), null, COMBAT_MESSAGE_RANGE, null)
...
/obj/effect/embers/proc/DamageCheck(mob/living/L)
	...
	if(prob(chance))
		to_chat(L, span_userdanger("一团燃烧的火焰附着在你身上!"))
