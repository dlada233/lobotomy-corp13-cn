...
/mob/living/simple_animal/hostile/abnormality/little_prince/proc/Hypno(mob/living/carbon/human/user)
	if (!(user.sanity_lost))
		...
		user.deal_damage(user.maxSanity, WHITE_DAMAGE, attack_type = (ATTACK_TYPE_SPECIAL))
		if (!(user.sanity_lost))
...
/obj/effect/prince_mushrooms/proc/DoDamage(mob/living/carbon/human/H)
	H.deal_damage(rand(1, 3), WHITE_DAMAGE, attack_type = (ATTACK_TYPE_ENVIRONMENT))
